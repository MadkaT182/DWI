...
if GAMESTATE:IsPlayerEnabled(PLAYER_1) then
	t[#t+1] = Def.ActorFrame {
		...
		LoadActor("Grade_E_Tier")..{
			InitCommand=cmd(x,SCREEN_CENTER_X-230;y,SCREEN_CENTER_Y-140;diffusealpha,0);
			OnCommand=function(self)
				if(STATSMAN:GetAccumPlayedStageStats():GetPlayerStageStats("PlayerNumber_P1"):GetGrade()=="Grade_Tier07" or STATSMAN:GetAccumPlayedStageStats():GetPlayerStageStats("PlayerNumber_P1"):GetGrade()=="Grade_Failed") then
					(cmd(sleep,.901;linear,.134;diffusealpha,1))(self);
				end;
			end;
			OffCommand=cmd(linear,0.2;zoom,0);
...
if GAMESTATE:IsPlayerEnabled(PLAYER_2) then
	t[#t+1] = Def.ActorFrame {
		...
		LoadActor("Grade_E_Tier")..{
			InitCommand=cmd(x,SCREEN_CENTER_X+230;y,SCREEN_CENTER_Y-140;diffusealpha,0);
			OnCommand=function(self)
				if(STATSMAN:GetAccumPlayedStageStats():GetPlayerStageStats("PlayerNumber_P2"):GetGrade()=="Grade_Tier07" or STATSMAN:GetAccumPlayedStageStats():GetPlayerStageStats("PlayerNumber_P2"):GetGrade()=="Grade_Failed") then
					(cmd(sleep,.901;linear,.134;diffusealpha,1))(self);
				end;
			end;
			OffCommand=cmd(linear,0.2;zoom,0);
